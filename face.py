    ...
    for new_image in new:
        ...
        for i in range(len(encoding2)):
            ...
            if True in ifsame:
                ...
                # 保存图片
                region2.save(str(num-1) + '号同学.PNG')
...
def sign_in(sign_image,list_of_face_encodings,total_num):
    ...
    for p in range(len(sign_encodings)):
        ...
        if True not in ifsame1:#如果此人没出现过
            ...
            region3 = img3.crop((sign_locations[p][3], sign_locations[i][0], sign_locations[p][1], sign_locations[p][2]))
            # 保存图片
            region3.save(str(p) + '号新同学.PNG')
...
#这里是想把所有缺席同学照片显示到一张图上
    # if is_empty==0:
    #     img_notin=np.zeros(len(list_notin))
    #     for g in range(len(list_notin)):
    #         img_notin[g]=cv.imread(str(list_notin[g])+'号同学.PNG')
    #
    #     htitch = np.hstack((img_notin[0],img_notin[1]))
    #     cv.imshow("缺席同学照片", htitch)
    #     cv.waitKey(0)
    #     cv.destroyAllWindows()


numbers,encodings=build_set(new,image)
sign_in(sign_image,encodings,numbers)
